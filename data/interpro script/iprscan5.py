...
def printGetParameterDetails(paramName):
    ...
    print(str(doc.name) + u"\t" + str(doc.type))
    print(doc.description)
    if hasattr(doc, 'values'):
        for value in doc.values:
            print(value.value)
            if str(value.defaultValue) == u'true':
                print(u'default')
            print(u"\t" + str(value.label))
            if hasattr(value, u'properties'):
                for wsProperty in value.properties:
                    print(u"\t" + str(wsProperty.key) + u"\t" + str(wsProperty.value))
...
def getResult(jobId):
    ...
    for resultType in resultTypes:
        ...
        if options.outfile:
            filename = (options.outfile + u'.' + str(resultType[u'identifier']) +
                        u'.' + str(resultType[u'fileSuffix']))
        else:
            filename = (jobId + u'.' + str(resultType[u'identifier']) +
                        u'.' + str(resultType[u'fileSuffix']))
        # Write a result file
        ...
        for outformat_type in outformat_parm:
            ...
            if not outformat_type or outformat_type == str(resultType[u'identifier']):
                if outputLevel > 1:
                    print("Getting %s" % str(resultType[u'identifier']))
                # Get the result
                result = serviceGetResult(jobId, str(resultType[u'identifier']))
                if (str(resultType[u'mediaType']) == u"image/png"
                        or str(resultType[u'mediaType']) == u"image/jpeg"
                        or str(resultType[u'mediaType']) == u"application/gzip"):
                    fmode = 'wb'
                else:
                    fmode = 'w'
